...
MemSaver 是一个 C++/CUDA 库，提供自研 CUDA caching allocator 和带 region 感知的内存池。它可以把一个带 tag 的 region 内的分配路由到独立 pool，在保持虚拟地址稳定的前提下 pause 和 resume 已接管的 GPU 显存，并支持 regular 与 arena 两种分配模式。
...
- 提供 regular、arena、arena-virtual 和模型加载运行时测试
...
- CMake 3.20 及以上
- C++17
- 构建测试时需要能被 CMake 找到的 PyTorch 或 LibTorch

`CMakeLists.txt` 会先尝试 `find_package(Torch)`，如果失败，再通过 `python -c "import torch; print(torch.utils.cmake_prefix_path)"` 查找运行时测试所需的 Torch CMake 包。
...
- `leave_region()` 会停止把后续分配路由到该 region 的 pool，并把 pool 释放回 MemSaver allocator，但缓存项仍可复用，直到显式调用 `evict_region_pool_from_cache(...)`
...
- 受管 region 与默认 pool 混合出现时的 matmul 场景
- 同线程与子线程下的 region 行为
- 重入同一 region 后的地址复用
- arena 共同底层映射
- arena offset 激活
